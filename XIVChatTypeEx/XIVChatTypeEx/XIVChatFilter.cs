...
{ // @formatter:wrap_array_initializer_style chop_always
    private static readonly (Group?, Group?, Channel)[][][] FilterMasks = new (Group?, Group?, Channel)[14][][]
    {
        new (Group?, Group?, Channel)[27][]
        {
            ...
            new (Group?, Group?, Channel)[2]
            {
                (null, null, Channel.TellOutgoing),
                (null, null, Channel.TellIncoming),
            },
            new (Group?, Group?, Channel)[2]
            {
                (null, null, Channel.Party),
                (null, null, Channel.CrossWorldParty),
            },
    ...
    {
        var filters = FilterMasks[category][index];
        foreach (var filter in filters) Remove(filter);
    }

    public void Remove((Group?, Group?, Channel) filter)
    {
        foreach (var item in ExpandFilter(filter)) _filterList.Remove(item);
    }
